...
- `goals.sh`: The core CLI.
  ...
  - `evaluate "<action>"`: Uses `gte-go` semantic vector search via the Neural Brain API to calculate the cosine similarity between the proposed action and active goals. Returns a float score.
